...
const SLADashboard = ({ data, onApiClick }) => {
  ...
  // Backend returns: { overallCompliance, apisWithSLA, compliantApis, nonCompliantApis, details }
  const compliant = data.compliantApis ?? 0;
  const nonCompliant = data.nonCompliantApis ?? 0;
  const averageUptime = parseFloat(data.overallCompliance) || 0;
  const apis = (data.details || []).map((d) => ({
    _id: d.apiId,
    apiName: d.apiName,
    slaConfig: d.slaConfig,
    latestReport: d.latestReport
      ? {
          compliance: d.latestReport.compliance,
          metrics: { uptime: d.latestReport.uptime },
        }
      : null,
  }));
