...
const OptimizationTips = ({ tips }) => {
  ...
  const normalizeTips = (rawTips) => {
    if (!rawTips || !Array.isArray(rawTips) || rawTips.length === 0) {
      return defaultTips;
    }
    ...
    return rawTips.map((tip) => {
      if (tip.message) {
        return {
          icon: TIP_TYPE_ICONS[tip.type] || Lightbulb,
          title: tip.title || "Optimization Tip",
  ...
  const displayTips = normalizeTips(tips);

  return (
    <div className='bg-linear-to-br from-indigo-900 to-indigo-950 rounded-[20px] p-6 text-white relative overflow-hidden shadow-lg'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4 relative z-10'>
        {displayTips.map((tip, idx) => {
          ...
          return (
            ...
            >
              <div
                className={`p-2 rounded-lg h-fit w-fit mb-3 ${
                  isCritical
                    ? "bg-red-500/30"
                    : isWarning
                      ? "bg-amber-500/30"
