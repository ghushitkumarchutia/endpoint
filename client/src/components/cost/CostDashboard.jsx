...
const CostDashboard = ({ data, onApiClick }) => {
  ...
  // Extract values with proper fallbacks from backend structure
  const totalCost = data.totalCost || 0;
  const budget = data.config?.budget || 0;
  const apis = data.topCostApis || [];
  const trend =
    data.costTrend === "increasing" ? 1 : data.costTrend === "stable" ? 0 : -1;
  const currency = data.config?.currency || "USD";
