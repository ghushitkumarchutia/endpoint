...
const SLATracking = () => {
  ...
  const handleGenerateReport = async () => {
    try {
      await generateAllReports(period);
      toast.success("Reports generated for all SLA-enabled APIs");
      fetchDashboard();
    } catch {
  ...
  const handleConfigSave = async () => {
    // Config is saved per-API, not globally - hide config panel for now
    toast.success("SLA configuration is managed per-API in API settings");
    setShowConfig(false);
  };
        ...
        <div className='bg-white border border-gray-200/60 rounded-[24px] p-6 shadow-sm'>
          <div className='flex items-center justify-between mb-6'>
            ...
            <span className='text-sm text-gray-500'>
              Select an API from above to view its reports
            </span>
          </div>
