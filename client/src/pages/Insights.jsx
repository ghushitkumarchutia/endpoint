...
const Insights = () => {
  ...
  const timelineEvents =
    [...(rootCauses || []), ...(predictiveAlerts || [])]
      .sort(
        (a, b) =>
          new Date(b.analyzedAt || b.createdAt) -
          new Date(a.analyzedAt || a.createdAt),
      )
      .slice(0, 10)
      .map((item) => ({
        id: item._id,
        timestamp: item.analyzedAt || item.createdAt,
        title: item.apiId?.name || item.apiName || "Unknown API",
        description:
          item.aiAnalysis || item.aiPrediction || "No details available",
        severity:
          item.failureProbability >= 70
            ? "critical"
            : item.failureProbability >= 50
              ? "error"
              : "warning",
      })) || [];
  ...
  return (
    <div className='flex flex-col px-4 py-5 md:px-6 md:py-5.5 bg-[#f5f5f6] rounded-3xl h-full overflow-y-auto custom-scrollbar'>
      ...
      <div className='animate-fade-in-up min-h-100'>
        {activeTab === "root-cause" && (
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-5'>
            {rootCauses?.length > 0 ? (
              ...
              <div className='col-span-1 lg:col-span-2 flex flex-col items-center justify-center py-16 text-center bg-white border border-gray-200/60 rounded-3xl shadow-sm border-dashed'>
        ...
        {activeTab === "timeline" && (
          <div className='bg-white border border-gray-200/60 rounded-3xl p-6 shadow-sm'>
            {timelineEvents.length > 0 ? (
              <InsightTimeline timeline={timelineEvents} />
            ) : (
